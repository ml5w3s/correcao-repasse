...
// Adiciona o método appliesTo() a cada objeto antes do freeze
for (const rule of rules) {
  rule.appliesTo = function(date) {
    const start = this.startDate || new Date(-8640000000000000); // início mínimo
    const end = this.endDate || new Date(8640000000000000);      // fim máximo
    return date >= start && date <= end;
  };
}

export const INTEREST_RULES_DEFINITIONS = Object.freeze(rules);

export function getInterestDefinitionForDate(date, definitions = INTEREST_RULES_DEFINITIONS) {
  for (const def of definitions) {
    if (def.appliesTo(date)) {
      return def;
    }
  }
  return null;
